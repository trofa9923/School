...
class BISchoolTeacher(models.Model):
    ...
    _sql_constraints = [
        ('teacher_no_uniq', 'unique(student_no)', "Teacher ID Number already exists !"),
    ]
    ...
    teacher_no = fields.Char('Teacher ID No.')
    ...
    subject_id = fields.Many2one('bi.school.subject.config', string="Subject")
